...
async function bootstrap() {
  ...
  // URI versioning only (see docs/api/versioning.md):
  //   - Versioned controllers (@Controller({ path, version: '1' })) are
  //     served at /api/v{N}/<resource>.
  //   - Controllers without a `version` are served at /api/<path> and are
  //     reachable under any future version number.
  //   - Infrastructure endpoints stay at the root so container health checks
  //     and Prometheus scrapes are unaffected by API versioning.
  app.setGlobalPrefix('api', {
    exclude: ['health', 'metrics', 'api/version-info'],
  });
  ...
  app.useGlobalInterceptors(
    new VersioningInterceptor(customLogger),
