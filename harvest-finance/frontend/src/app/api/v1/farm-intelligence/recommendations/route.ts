...
export async function GET(request: NextRequest) {
  ...
  const preferredCrop = searchParams.get('preferredCrop') || undefined;
  const soilType = searchParams.get('soilType') || undefined;
  const farmSize = searchParams.get('farmSize') || undefined;
  ...
  const response = buildRecommendations(input);
