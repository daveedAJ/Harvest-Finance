...
.PHONY: help dev dev-frontend dev-all build build-frontend \
        lint format typecheck docs \
        test test-cov test-e2e test-watch \
        db-up db-down db-migrate db-migrate-revert db-seed db-seed-clear db-seed-reset \
...
typecheck: ## Type-check the backend (tsc --noEmit)
	cd $(BACKEND_DIR) && npm run typecheck

docs: ## Generate backend code documentation with Compodoc
...
db-down: ## Stop the Docker Compose database services
	docker compose down
